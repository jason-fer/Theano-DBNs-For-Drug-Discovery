...
def load_files(activeFile, inactiveFile):
    #print '... Loading files'
    foldsActive = {0:[],1:[],2:[],3:[],4:[]}
    foldsInactive = {0:[],1:[],2:[],3:[],4:[]}

    fileHandle = open(activeFile, 'r')
    rowCount = 0
    for line in fileHandle:
        ...
        tuple = (rowCount, fold, compoundName, classLabel, fingerprintArray)
        foldsActive[fold].append(tuple)

    numActives = rowCount

    fileHandle = open(inactiveFile, 'r')
    #print "Processing Inactive File: ", inactiveFile
    for line in fileHandle:
        rowCount += 1
        words = line.split()
        classLabel = int(words[1])
    ...
    numInactives = rowCount - numActives
    multiplier = int(numInactives/numActives) + 1
    print "Actives, Inactives, Multiplier: ", numActives, numInactives, multiplier
...
def load_data(testFold, foldsActive, foldsInactive, multiplier):
    ...
    trainList, trainFingerprintArrayList, trainClassLabelList = [], [], []

    for item in listTrainActive:
        tuple = (numpy.array(item[4]), item[3])
    ...
    testList, testClassLabelList, testFingerprintArrayList = [], [], []
    ...
    #print "Rows: Train, Test: ", rows_train, rows_test
    return rval

def sgd_optimization(testFold, foldsActive, foldsInactive, multiplier):
    """
    ...
    datasets = load_data(testFold, foldsActive, foldsInactive, multiplier)
    ...
    #print "Number of Mini-batches: Train, Valid, Test: ", n_train_batches, n_valid_batches, n_test_batches
...
if __name__ == '__main__':
    readPath = "/home/jferiante/filenames.txt"
    ...
    for line in listOfLines:
        ...
        foldsActive, foldsInactive, multiplier = load_files(activeFile, inactiveFile)
        for fold in range(0,5):
            sgd_optimization(fold, foldsActive, foldsInactive, multiplier)
